...
class Monitor(xbmc.Monitor):
   ...
   def createPlayerConfPickle(self):
      ...
      file = open(os.path.join(self.path,'acestream.engine','androidfs','acestream.engine','.ACEStream','playerconf.pickle'), 'wt')
   ...
   def createAcestreamConf(self):
      ...
      if self.param('is_debug') == 'true':
         ...
         arg += '--log-debug\n0'
      file = open(os.path.join(self.path,'acestream.engine','androidfs','acestream.engine','acestream.conf'), 'wt')
      file.write(arg)
      file.close()
