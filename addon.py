...
def get_disks():
   disks=[]
   script_ismount = os.path.join(xbmcaddon.Addon().getAddonInfo('path'), 'acestream.engine','mount_disk.sh')
   if os.path.exists(script_ismount):
      ...
      for name in names:
         i=names.index(name)
         if (i%2 == 1):
            if name != '/storage/.kodi/addons/service.system.acestreammedia/acestream.engine/androidfs/storage':
